I - How to execute the application:

    - Before running any script, there must be an active Apache Zookeeper server on the local network. 
    This can be done by executing the following command in the directory where Apache Zookeeper is installed:
        $ bin/zkCli.sh
        
    - Apache Zookeeper download link (version 3.8.4): 
        https://www.apache.org/dyn/closer.lua/zookeeper/zookeeper-3.8.4/apache-zookeeper-3.8.4-bin.tar.gz

    - Then, the application server script must be executed, through the command:
        $ python3 kuko_server.py [host] [port]

    - After this, several clients can use the application in the terminal through the command:
        $ python3 kuko_client.py [id_participant] [host] [port]

    - To serve the application in Flask, the following script must be executed through the command:
...
II - HOW TO USE THE APPLICATION: 
	...
	Some examples of commands are:
        ...
        QUESTION:
            $ 10;What's the capital of Germany?;Frankfurt am Main;Berlin;Cologne;Munich;2
        QSET: 
            $ 20;4;5;6;7;8
        QUIZ:
            $ 30;3;5;5;5;5;5
	...
	through the POST and GET methods. Some examples are:
        ...
        - curl with POST method, to perform the QUESTION operation:
            $ curl -X POST -H "Content-Type: application/json" -d 
            '{"question": "What's the capital of Australia?", 
            "answers": "Canberra;Melbourne;Sydney;Perth;Brisbane", 
            "k":2}' http://127.0.0.1:5005/question

        - curl with GET method, to perform the REL operation:
            $ curl -G -d "quiz=1" http://127.0.0.1:5005/rel

        - URL with GET method to perform the GET operation:
...
III - OTHER INFORMATION: 
	...
	This extension needs to be installed separately from the Flask framework with the following command: 
